...
class TestCalculateMovingAverages(unittest.TestCase):
    ...
    def test_matches_pandas_rolling_means(self):
        ...
        self.assertAlmostEqual(
            result["ma10"], float(expected_ma10), places = 7,
        )
        self.assertAlmostEqual(
            result["ma20"], float(expected_ma20), places = 7,
        )
    ...
    def test_short_series_yields_nan_for_large_windows(self):
        ...
        self.assertTrue(
            pd.isna(result["ma3"]),
        )  # not enough points for window=3
        self.assertTrue(pd.isna(result["ma10"]))
        self.assertTrue(pd.isna(result["ma20"]))
